...
🎯 BASIC USAGE (Lines 25-56)
- Default implementation with flexbox layout
- Default active state management
- Custom default active item

🔗 URL ROUTING (Lines 141-274)
- React Router v7/v6 integration
- Automatic URL updates and active state sync
- Nested routes, custom paths, base path prefix
- Custom navigation handler (escape hatch)
- Disable routing for testing

📐 LAYOUT INTEGRATION (Lines 276-322)
- Recommended Layout Pattern (Lines 62-84): Flexbox setup
- Right Position (Lines 86-104): Alternative positioning

🔄 EXPANSION BEHAVIOR (Lines 106-154)
...
🌍 INTERNATIONALIZATION (Lines 373-587)
...
♿ ACCESSIBILITY (Lines 589-612)
- Keyboard navigation support
- ARIA labels and screen reader compatibility
- Focus management

📏 SIZING & CUSTOMIZATION (Lines 614-662)
...
💼 COMMON USE CASES (Lines 664-738)
...
⚙️ TECHNICAL DETAILS (Lines 740-781)
...
🔄 MIGRATION GUIDES
- React Router Migration (Lines 972-1049): Breaking change guide, troubleshooting
- Fixed Layout Migration (Lines 1051-1063): Flexbox transition
...
The Tagaddod Design System provides **two Sidebar components** to support different navigation needs:

###  Core Sidebar (Callback-Based Navigation)

The core `Sidebar` component works **without React Router** using callback-based navigation. Best for:
- Applications not using React Router
- Custom routing solutions
- Static sites or SPAs with manual state management
...
function App() {
  ...
  const handleNavigation = (itemId) => {
    ...
    // e.g., update state, fetch data, switch views, etc.
  };
    ...
    <div style={{ display: 'flex', height: '100vh' }}>
      <Sidebar
        ...
        menuItems={[
          ...
          { id: 'products', icon: IconPackage, label: 'Products' }
        ]}
      />
      <main style={{ flex: 1 }}>
...
### 📦 SidebarWithRouter (React Router Integration)

The `SidebarWithRouter` component integrates with React Router v7/v6 to automatically update the browser URL and sync active states.

**Requirements:**
1. Install React Router: `npm install react-router@^7.0.0`
...
- Clicking "Dashboard" → URL changes to `/dashboard`
- Clicking "Products" → URL changes to `/products`

**Automatic Active State:**
- URL `/dashboard` → "Dashboard" menu item highlighted
- Works on initial page load and browser back/forward buttons

### Nested Routes Example

Child menu items create nested URL paths:
...
const menuItems = [
  {
    ...
    hasChildren: true,
    children: [
      { id: 'all', label: 'All Products' },      // → /products/all
      { id: 'add', label: 'Add Product' },       // → /products/add
      { id: 'categories', label: 'Categories' }  // → /products/categories
    ]
  }
];
...
Override auto-generated paths using the `path` prop:

```jsx
const menuItems = [
  { id: 'home', path: '/', label: 'Home' },              // Use "/" instead of "/home"
  { id: 'config', path: '/settings', label: 'Config' }   // Use "/settings" instead of "/config"
];
...
Prefix all routes with a base path (useful for admin panels, multi-tenant apps):

```jsx
<SidebarWithRouter
  basePath="/admin"
  menuItems={[
    { id: 'dashboard', label: 'Dashboard' }  // → /admin/dashboard
  ]}
/>
```
...
Add custom logic before navigation (analytics, auth checks, etc.):
...
<SidebarWithRouter
  ...
  onNavigate={(path, itemId) => {
    ...
    trackAnalytics('sidebar_click', { page: itemId });
...
### Disable Routing (Testing/Storybook)

Disable URL routing for isolated testing:
  ...
  const menuItems = [
    {
      id: 'navigation',
      icon: BackIcon,
      label: isRTL ? 'التنقل' : 'Navigation',
      hasChildren: true,
      children: [
        { 
          id: 'next-page', 
          label: isRTL ? 'الصفحة التالية' : 'Next Page',
...
### Breaking Change: React Router Required

**Version 0.1.30+** requires React Router to be installed.

#### Step 1: Install React Router

```bash
# React Router v7 (recommended for 2025)
npm install react-router@^7.0.0

# OR React Router v6 (also supported)
...
Modern web apps expect URL updates when navigating. This change:
- ✅ Enables shareable URLs (send `/dashboard` link to coworkers)
...
Solution: Wrap your app with `<BrowserRouter>`:
```jsx
import { BrowserRouter } from 'react-router';
...
**Menu items not matching URL on load**

Ensure menu item `id` values match your route paths:
```jsx
// Menu item
...
**Need to disable routing for testing?**

Use the `disableRouting` prop:
```jsx
<Sidebar disableRouting={true} menuItems={menuItems} />
```
